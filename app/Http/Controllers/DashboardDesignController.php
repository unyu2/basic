...
class DashboardDesignController extends Controller
{
    ...
    public function index()
    {
        ...
        while (strtotime($tanggal_awal) <= strtotime($tanggal_akhir)) {
            ...
            $temuan_closeds= Design::where ('status','Release')->where('id_proyek','123')->where('created_at', 'LIKE', "%$tanggal_awal%")->count();
            $temuan_opens= Design::where ('status','Open')->where('id_proyek','123')->where('created_at', 'LIKE', "%$tanggal_awal%")->count();
