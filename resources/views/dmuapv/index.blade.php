...
@section('title')
    Approve Formulir Check Sheet Untuk DMU / EMU
@endsection

@section('breadcrumb')
    @parent
    <li class="active">Approve Formulir Check Sheet Untuk DMU / EMU</li>
@endsection
    ...
    <div class="col-lg-12">
        <div class="box">
         ...
         <!--       <button onclick="addForm('{{ route('dmu.storeModal') }}')" class="btn btn-success btn-flat"><i class="fa fa-plus-circle"></i> Buat Form Inspeksi Modal</button> -->
                    <button onclick="deleteSelected('{{ route('dmuapv.delete_selected') }}')" class="btn btn-danger btn-flat"><i class="fa fa-trash"></i> Hapus</button>
                </div>
            </div>
...
@includeIf('dmuapv.form3')
@includeIf('dmuapv.form2')
...
<script>
    ...
    window.location = "{{ route('dmuapv.index') }}";
    }
    ...
    $('#modal-form3 [name=nama_dmu]').focus();
        ...
        $.get(url)
            ...
            $('#foto14-preview').attr('src', response.foto14 ? response.foto14 : '');
                ...
                $('#modal-form3 [name=status]').val(response.status);
